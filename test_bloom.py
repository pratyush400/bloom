"""The false poisitve rate of normal bloom filter is 18.7% 1st time 17.8% 2nd time so average around 18%"""
import pytest
...
def test_false_positive_rate_is_low(filter_):
    for domain in BAD:
        filter_.add(domain)
    print(len(list(filter(filter_.might_contain, GOOD)))/len(GOOD))
